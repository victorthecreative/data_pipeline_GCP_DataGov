  ...
  tables=[
    {
        ...
        clustering      = ["produto","sigla_regiao", "estado_sigla"],
        labels          = {
          name    = "datagov_data_pipeline"
          project  = "gasolina"
        },
...
module "bucket-raw" {
  ...
  name       = "data-pipeline-datagov-combustiveis-brasil-raw"
  project_id = var.project_id
  location   = var.region
}
...
module "bucket-curated" {
  ...
  name       = "data-pipeline-datagov-combustiveis-brasil-curated"
  project_id = var.project_id
  location   = var.region
}
...
module "bucket-pyspark-tmp" {
  ...
  name       = "data-pipeline-datagov-combustiveis-brasil-pyspark-tmp"
  project_id = var.project_id
  location   = var.region
}
...
module "bucket-pyspark-code" {
  ...
  name       = "data-pipeline-datagov-combustiveis-brasil-pyspark-code"
  project_id = var.project_id
  location   = var.region
}